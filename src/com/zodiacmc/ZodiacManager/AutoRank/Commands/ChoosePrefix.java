...
public class ChoosePrefix extends SubCommand {
	...
	public boolean processCommand(CommandSender sender, String[] args) {
		...
		if (args.length == 0) {
			...
			return CommandUtil.success(sender, command.getPrefix() + " &aUsage: /AutoRank ChoosePrefix <Number>");
		}
		if (args.length != 1)
			return CommandUtil.success(sender, command.getPrefix() + " &aUsage: /AutoRank ChoosePrefix <Number>");
		Integer value = null;
		...
		if (value < 0)
			return CommandUtil.success(sender, command.getPrefix() + " &cError: Number must be greater than 0.");
		if (prefixes.size() < value+1)
			return CommandUtil.success(sender, command.getPrefix() + " &cError: Number must be smaller than " + (prefixes.size()+1) + ".");
		Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(), "manuaddv " + player.getName() + " prefix " + prefixes.get(value));
		return CommandUtil.success(sender, command.getPrefix() + " &aPrefix has been successfully updated to: " + prefixes.get(value));
	}
